...
data class ContentRating(
    ...
    class Rating(val name: String, val minAge: Double, val aliasList: List<String> = emptyList()) {
        constructor(name: String, minAge: Double, vararg alias: String) : this(name, minAge, listOf(*alias))
    }
    ...
    companion object {
        ...
        private val COUNTRY_RATING_MAP = mapOf(
            "AU" to listOf(
                ...
                Rating("PG", 7.5),
                Rating("M", 15.0),
                Rating("MA15+", 16.5, "AV15+"),
                Rating("R18+", 18.0)
            ),
            "CA" to listOf(
                Rating("G", 0.0, "C", "Exempt"),
                Rating("PG", 8.0, "C8"),
                Rating("14+", 14.0),
                Rating("18+", 18.0)
            ),
            ...
            "US" to listOf(
                Rating("TV-G", 0.0, "TV-Y", "NR"),
                Rating("TV-PG", 7.0, "TV-Y7"),
                Rating("TV-14", 14.0),
                Rating("TV-MA", 17.0)
            )
        )
        ...
        private fun calculateRating(countryCode: String, contentRatingList: List<ContentRating>): String {
            return COUNTRY_RATING_MAP[countryCode]?.let { ratingList ->
                contentRatingList
                    ...
                    .map { toMinAge(it.rating, COUNTRY_RATING_MAP.getValue(it.countryCode)) }
                    .average()
                    .takeIf { it.isFinite() }
                    ?.let { "Calculated Rating " + toRatingString(it, ratingList) }
